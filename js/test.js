...
//alert(window.secureKeysArray[chatId]);
			...
			if (decryptedHTML == null || decryptedHTML == '')
				decryptedHTML = '<b>ERROR</b> (this message can be encoded by another Secret Key)';
...
//alert ('selected chat id: ' + chatId); //ïîäñêàçêà äëÿ îïðåäåëåíèÿ ID ÷àòà
			}

		}

		if (document.getElementById('secureForms')) {
		} else {
...
//àëåðò äëÿ îïðåäåëåíèÿ ñåêðåòíîãî êëþ÷à
...
//åñëè â îêíå íå îòðèñîâàíà êíîïêà è îêíî íå ñåêðåòíî
...
//ñîçäàžì ñåêðåòíóþ ôîðìó, åñëè åž íåò
			if (document.getElementById('secureForm' + chatId)) {
			} else
				document.getElementById('secureForms').innerHTML = document.getElementById('secureForms').innerHTML + '<textarea id="secureForm' + chatId + '" style="height: 40px; width: 348px; padding: 3px 5px 5px 3px; display: block; font: normal normal 400 11px/16px Tahoma; border: 1px solid #C0CAD5; z-index: 140;  position: absolute; top:0; resize: none; " ></textarea>'
...
//åñëè áûëà îòðèñîâàíà êíîïêà ââåäèòå êëþ÷ - óäàëèòü åž
			if (document.getElementById('sucureButtonOn')) {
				element = document.getElementById("sucureButtonOn");
				element.parentNode.removeChild(element);
			}

//åñëè êíîïêà ââåäèòå êëþ÷ íå áûëà îòðèñîâàíà
